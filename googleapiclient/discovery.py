...
from googleapiclient import six
...
def build(serviceName,
  ...
  for discovery_url in (discoveryServiceUrl, V2_DISCOVERY_URI,):
    ...
    try:
      ...
      if e.resp.status == six.moves.http_client.NOT_FOUND:
        continue
      else:
        raise e
...
Returns:
  ...
  page. Returns None if there are no more items in the collection.
    ...
    parsed = list(six.moves.urllib.parse.urlparse(request.uri))
    q = six.moves.urllib.parse.parse_qsl(parsed[4])
